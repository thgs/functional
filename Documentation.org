...
*** Monad
** Wrap existing code
A convenience class is provided called ~Wrapper~ to help wrap around
existing code. Creating the ~Wrapper~ by calling
~Wrapper::withAdjustedInput~ will automatically reflect on the callable
and assume input in a Tuple or Tuple3 (2 or 3 arguments).

Then you can further
adjust the input by calling `adjustInput` or `contramap` directly.
...
Note that in ~adjustInput~ we need to accept a Tuple or Tuple3 if the
initial arguments are 2 or 3. More than 3 arguments are not currently
supported. In the case of 0 or 1 arguments there is no need to use any
tuple class.
...
#+begin_src php
  ...
  $wrapper = Wrapper::withAdjustedInput( fn (Tuple $p) => $psr3logger->critical($p->fst(), [$p->snd()]) );

  $wrapper( t("Log message", "context") );

#+end_src

You may construct the wrapper without any input adjustment by using
the constructor directly. This will skip reflection and adjustment of
input to a Tuple.

#+begin_src php

  $wrapper = new Wrapper($psr3logger->critical(...));

  $wrapper("Log message", ["context"]);

#+end_src

The ~Wrapper~ supports adjusting the output as well by calling
~adjustOutput~ or directly ~fmap~.

#+begin_src php 

  $wrapper = new Wrapper($psr3logger->critical(...)); // returns void
...
The above works because $psr3Logger->critical() returns ~void~. Here is
a different example to demonstrate adjusting the output without
adjusting the input.
...
#+begin_src php
  ...
  print $result;  // Prints "The result is 14\n"
...
are the parameters for initially wrapped thing.

If for some reason the adjustment of input or output that the ~Wrapper~
provides is not the desired one, then one must implement their own
