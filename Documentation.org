...
this library. For example `lamphpda` has different objects for each use-case:
there are distinct implementations of MaybeFunctor, MaybeApplicative etc because
the design there holds different things invariant (or fixed or puts gravity on)
from the design of this library.
